...
from analyze_gse202379_programs import singscore_up
from audit_gse181483_gates import contrast_groups


def descriptive_effect(control: np.ndarray, case: np.ndarray) -> dict[str, float | int | str]:
    n_control = len(control)
    n_case = len(case)
    pooled_variance = (
        ((n_control - 1) * control.var(ddof=1) + (n_case - 1) * case.var(ddof=1))
        / (n_control + n_case - 2)
    )
    pooled_sd = float(np.sqrt(pooled_variance))
    mean_difference = float(case.mean() - control.mean())
    cohens_d = mean_difference / pooled_sd if pooled_sd > 0 else np.nan
    correction = 1 - 3 / (4 * (n_control + n_case) - 9)
    return {
        "n_control": n_control,
        "n_case": n_case,
        ...
        "uncertainty_status": "not estimated: n=2 per group directional display only",
        "permutation_status": "not tested: frozen no standalone p-value boundary",
    }
...
def main() -> None:
    ...
    coverage = pd.read_csv(repo / "results" / "qc" / "gse181483_program_coverage.csv")
    ...
    score_path = sensitivity_dir / "gse181483_donor_program_scores.csv.gz"
    scores.to_csv(score_path, index=False, compression="gzip")

    gates = pd.read_csv(repo / "results" / "qc" / "gse181483_donor_gate_summary.csv")
    ...
    for contrast, groups in contrast_groups(manifest).items():
        for program_id, program_rows in programs.groupby("program_id"):
            ...
            if status["directional_30_cell_gate"] == "PASS":
                eligibility_column = "eligible_30"
                cell_gate = 30
            elif status["directional_20_cell_gate"] == "PASS":
            ...
            for score_method in ("singscore", "standardized_mean"):
                ...
                if len(control) != 2 or len(case) != 2:
                    raise RuntimeError("Directional effect must use exactly two donors per group")
                effect_rows.append(
                    {
                        "dataset_id": "GSE181483_human",
                        "contrast": contrast,
                        ...
                        "lineage": lineage,
                        "score_method": score_method,
                        "analysis_tier": "directional",
                        "cell_gate": cell_gate,
                        "coverage_tier": coverage_info["coverage_tier"],
                        "program_coverage": coverage_info["coverage"],
                        "expected_direction": "positive_case_higher",
                        "endpoint_limitation": "two donors per group; no standalone inferential claim",
                        **descriptive_effect(control, case),
                    }
                )
    effects = pd.DataFrame(effect_rows).sort_values(["lineage", "program_id", "score_method"])
    effect_path = sensitivity_dir / "gse181483_directional_effects.csv"
    ...
    summary = {
        ...
        "standardization_reference": "all four lineage donors passing 20-cell gate; outcome-blind",
        "endpoint": "two healthy versus two cirrhosis donors; directional display only",
        "formal_p_values": "not computed",
        "formal_intervals": "not computed",
    ...
    (repo / "results" / "logs" / "gse181483_analysis_run.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(effects[["lineage", "program_id", "score_method", "cell_gate", "hedges_g"]].to_string(index=False))
