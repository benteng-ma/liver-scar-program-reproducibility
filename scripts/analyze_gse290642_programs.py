...
from audit_gse290642_gates import contrast_groups
...
def main() -> None:
    ...
    if counts.shape != (len(union_genes), len(manifest)):
        raise RuntimeError("count matrix does not match gene/group manifests")
    shared_set = set(shared_genes)
    shared_rows = np.array(
        [index for index, gene in enumerate(union_genes) if gene in shared_set],
        dtype=int,
    )
    genes = union_genes.iloc[shared_rows].reset_index(drop=True)
    counts = counts[shared_rows, :]
    if set(genes) != shared_set or len(genes) != len(shared_genes):
        raise RuntimeError("shared feature subset is incomplete or duplicated")
    ...
    coverage = pd.read_csv(repo / "results" / "qc" / "gse290642_program_coverage.csv")
    ...
    score_path = sensitivity_dir / "gse290642_donor_program_scores.csv.gz"
    scores.to_csv(score_path, index=False, compression="gzip")

    gates = pd.read_csv(repo / "results" / "qc" / "gse290642_donor_gate_summary.csv")
        ...
        for program_id, program_rows in programs.groupby("program_id"):
            ...
            for score_method in ("singscore", "standardized_mean"):
                ...
                stats = effect_statistics(
                    control,
                    case,
                    stable_seed("GSE290642_human", contrast, program_id, score_method),
                )
                effect_rows.append(
                    {
                        "dataset_id": "GSE290642_human",
                        ...
                        "annotation_limitation": "reconstructed broad lineage; no author processed labels",
                        "etiology_limitation": "unresolved",
                        **stats,
                    }
                )
    effects = pd.DataFrame(effect_rows).sort_values(["contrast", "lineage", "program_id", "score_method"])
    effect_path = sensitivity_dir / "gse290642_sensitivity_effects.csv"
    ...
    run_summary = {
        ...
        "feature_space": f"{len(shared_genes)} genes measured in all 24 donors",
    ...
    (repo / "results" / "logs" / "gse290642_analysis_run.json").write_text(
        json.dumps(run_summary, indent=2) + "\n", encoding="utf-8"
    )
    print(effects[["contrast", "lineage", "program_id", "score_method", "cell_gate", "n_control", "n_case", "hedges_g", "robust_ci95_low", "robust_ci95_high", "permutation_p_two_sided"]].to_string(index=False))
